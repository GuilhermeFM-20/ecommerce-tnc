<?php if(!class_exists('Rain\Tpl')){exit;}?><div class="modal fade" id="exampleModal<?php echo htmlspecialchars( $value["id"], ENT_COMPAT, 'UTF-8', FALSE ); ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title font-weight-normal" id="exampleModalLabel">Aviso!</h5>
        <button type="button" class="btn-close text-dark" data-bs-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        Deseja realmente excluir esse registro?
      </div>
      <div class="modal-footer">
        <button type="button" class="btn bg-gradient-secondary" data-bs-dismiss="modal">Não</button>
        <a href="/categoria/delete/<?php echo htmlspecialchars( $value["id"], ENT_COMPAT, 'UTF-8', FALSE ); ?>" id="link_delete"><button type="button" class="btn bg-gradient-primary">Sim</button></a>
      </div>
    </div>
  </div>
</div>   